...
The My Tickets API contract requires a parseable `ticketDate` for every returned row. Client response validation rejects missing/invalid Ticket Date data before normal list rendering. The visible `—` fallback remains only as last-resort defensive rendering and does not redefine missing `ticketDate` as a valid application state.
...
artifacts/lab-02/screenshots/states/{requester-loading,requester-failure,loading,validation,invalid-attachment,submitting,success,failure,empty,no-results,removed-attachment}.png
```
