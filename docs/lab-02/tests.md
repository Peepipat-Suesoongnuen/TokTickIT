...
Companion to [specification.md](./specification.md) and [api-spec.md](./api-spec.md). Created before implementation as the Test DD / TDD plan. Feature behavior changes are developed failing-first where a behavior is not yet implemented. Final-integration work may also close a previously planned **evidence gap** for behavior that already exists; such a test can legitimately pass on its first execution and must be recorded as evidence closure rather than retroactively claimed as a Red→Green implementation cycle. This document keeps the original planned-test contract and records feature-branch evidence as implementation progresses (see §6). Rows remained `Planned` until the required final `main` verification; the current `Final` column now reflects that verified result.

**Status key:** `Planned` before final-main verification. A row becomes `Pass` only after the corresponding automated coverage is green on the final `main` release tree — never from feature-branch or staging-only evidence alone. Final-main release evidence is recorded in §6. `E2E-01` is marked `Pass (retry)` because the final-main Playwright run passed it on the configured retry after one timeout; this is disclosed rather than flattened into a clean first-pass result.
...
|---|---|---|---|---|---|---|
| UNIT-01 | Unit | AC-01, BR-01 | Ticket Number format `{YY}{MM}-0001` and monthly reset boundary | Correct format; sequence restarts at month change | server/src/lib/__tests__/ticket-number.test.ts | Pass |
| UNIT-02 | Unit | BR-01, Assumption 2 | Sequence increment; exhaustion signal at 9999 | Increments deterministically; exhaustion flagged for safe failure | server/src/lib/__tests__/ticket-number.test.ts | Pass |
| UNIT-03 | Unit | AC-02, AC-03 | Trim + length validation summary 5–120 / description 20–2,000 | Trimmed values validated per BR-10 | server/src/lib/__tests__/validation.test.ts | Pass |
| UNIT-04 | Unit | AC-04 | Requested Priority enum `LOW\|MEDIUM\|HIGH\|CRITICAL`; pure reference-id shape only if a production helper exists | Invalid priority rejected; DB reference existence/active validity remains API-06 evidence | server/src/lib/__tests__/validation.test.ts | Pass |

### API — `server/tests/lab-02/`

| ID | Type | Requirement / AC | What It Tests | Expected Result | Test File | Final |
|---|---|---|---|---|---|---|
| API-01 | API | AC-07 | GET /api/requesters returns only active requesters | Inactive requester absent from response | server/tests/lab-02/reference-data.api.test.ts | Pass |
| API-02 | API | AC-09 | GET /api/requesters when none active | 200 with `[]` | server/tests/lab-02/reference-data.api.test.ts | Pass |
| API-03 | API | FR-01 | GET /api/categories & /api/related-systems active-only, ordered by name | Ordered arrays of active records | server/tests/lab-02/reference-data.api.test.ts | Pass |
| API-04 | API | AC-01 | POST valid ticket | 201; backend-generated number; status NEW; saved values returned from DB | server/tests/lab-02/create-ticket.api.test.ts | Pass |
| API-05 | API | AC-02, AC-03 | Summary/description out-of-bounds (min & max) | 400 + fieldErrors with documented copy | server/tests/lab-02/create-ticket.api.test.ts | Pass |
| API-06 | API | AC-04 | Invalid categoryId / relatedSystemId / requestedPriority | 400 for each case | server/tests/lab-02/create-ticket.api.test.ts | Pass |
| API-07 | API | BR-08 | Ticket bound to submitted requesterId | Persisted requesterId matches the submitted requester at creation | server/tests/lab-02/create-ticket.api.test.ts | Pass |
| API-08 | API | AC-11 | Requester B lists tickets | None of Requester A's tickets appear | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-09 | API | AC-12 | Search: case-insensitive partial on Ticket Number OR Summary; Description-only term; whitespace-only value | Ticket Number/Summary matches returned; Description-only term excluded; whitespace-only → 400 | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-10 | API | AC-13 | Filter by active categoryId, requestedPriority, and currentStatus=`NEW`; reject inactive categoryId / invalid status | Only matching tickets returned; inactive category or invalid status → 400 | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-11 | API | AC-14, BR-21 | Default updatedAt DESC + id DESC; priority rank; Ticket Number ASC/DESC; Ticket Date/Created ASC/DESC | Ordering matches contract | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-12 | API | AC-15 | Pagination metadata; page sizes {10,20,50}; page > totalPages → 200 empty data with valid meta | Correct meta fields per api-spec §5.2 | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-13 | API | AC-16 | Strict query contract: unknown param name → 400; invalid known value → 400 | Exact statuses per api-spec §5.2 | server/tests/lab-02/my-tickets.api.test.ts | Pass |
| API-14 | API | AC-10 | Detail of A's ticket requested as B; nonexistent id | 404 safe envelope both cases | server/tests/lab-02/ticket-detail.api.test.ts | Pass |
| API-15 | API | FR-08 | Owned detail includes removed attachment metadata | 200; removed rows show removedAt/removedReason | server/tests/lab-02/ticket-detail.api.test.ts | Pass |
| API-16 | API | AC-18 | Valid upload (permitted type ≤ 5 MB) | 201 + metadata; active record created | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-17 | API | AC-19 | Wrong type → 415; oversize > 5 MB → 413 | Exact statuses; no record created | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-18 | API | AC-20 | Upload when 5 active attachments exist | 409 Conflict | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-19 | API | AC-21, BR-09 | Remove owned attachment with reason; B removes A's attachment (ownership enforcement) | 200 retained metadata / 404 safe | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-20 | API | BR-17 | Download after removal | Blocked with 404 | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-21 | API | AC-22 | Remove without reason / blank reason | 400 + fieldError | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-22 | API / integration | AC-23, BR-18 | Create Ticket succeeds, one attachment upload succeeds, later upload is rejected | Ticket remains retrievable; successful upload remains active; rejected file creates no attachment | server/tests/lab-02/initial-attachment-failure.api.test.ts | Pass |
| API-23 | API | api-spec §7 | Forced unexpected server fault via mocked Prisma/service dependency (`vi.mock`), real route and error middleware active | 500 with safe generic envelope; no stack traces or internals in response | server/tests/lab-02/create-ticket.api.test.ts | Pass |
| API-24 | API | AC-24 | Requester B retrieves **metadata** of A's attachment | 404 safe envelope | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-25 | API | AC-24 | Requester B **downloads** A's active attachment | 404 safe envelope | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-26 | API | FR-10 | Owner retrieves **metadata** of owned active attachment | 200 correct shape (originalFilename, mimeType, sizeBytes, removedAt null) | server/tests/lab-02/attachments.api.test.ts | Pass |
| API-27 | API | FR-10 | Owner **downloads** owned active attachment | 200 binary stream; `Content-Type` = stored mime; `Content-Disposition: attachment; filename="<originalFilename>"` | server/tests/lab-02/attachments.api.test.ts | Pass |

Submission evidence for AC-25/AC-26 shows the safe failure state with entered values preserved. The Labsheet permits either stopping the backend **or simulating failure**; deterministic failure simulation already exists through component/API tests and `VISUAL-01`, while a live backend-stop demonstration may be added as optional extra evidence if useful for the final PDF/demo.

### UI Component — `client/src/features/lab-02/tests/`

| ID | Type | Requirement / AC | What It Tests | Expected Result | Test File | Final |
|---|---|---|---|---|---|---|
| UI-01 | UI | AC-02 | Summary out-of-bounds shows field-level message; form not submitted | Message under field; no request | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-02 | UI | AC-03 | Description out-of-bounds message | Message under field | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-03 | UI | AC-05 | Submit while pending | Busy/disabled; exactly one request | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-04 | UI | AC-26 | Submission failure | Safe banner; entered values preserved | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-05 | UI | AC-01 | Success state | Official Ticket Number rendered | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-06 | UI | AC-22 | Removal modal confirm blocked until non-empty reason | Confirm disabled while blank | client/src/features/lab-02/tests/AttachmentSection.test.tsx | Pass |
| UI-07 | UI | AC-17 | Empty vs no-results rendering | Distinct copy + CTA per state | client/src/features/lab-02/tests/MyTickets.test.tsx | Pass |
| UI-08 | UI | AC-12, AC-13 | Search/filter controls issue correct query params | Params match api-spec §5.2 | client/src/features/lab-02/tests/MyTickets.test.tsx | Pass |
| UI-09 | UI | AC-25 | Screen load failure | Safe error state + Retry | client/src/features/lab-02/tests/MyTickets.test.tsx | Pass |
| UI-10 | UI | AC-10 | Detail 404 view | "Ticket not found" + back link | client/src/features/lab-02/tests/RequesterTicketDetail.test.tsx | Pass |
| UI-11 | UI | AC-21 | Removed attachment row | Struck-through/muted; no Download action | client/src/features/lab-02/tests/RequesterTicketDetail.test.tsx | Pass |
| UI-12 | UI | AC-23 | Initial attachment upload failure after Ticket creation | Success state remains with official Ticket Number; failed filename/reason shown; Retry from Ticket Detail link points to created Ticket | client/src/features/lab-02/tests/CreateTicketAttachments.test.tsx | Pass |
| UI-13 | UI | AC-19 | Invalid file selected on Create Ticket | Local rejection reason + Dismiss shown; invalid file is excluded from attachment upload | client/src/features/lab-02/tests/CreateTicketAttachments.test.tsx | Pass |
| UI-14 | UI | BR-14 | Limit helper text at 5 active attachments | "Maximum of 5 active attachments reached" shown | client/src/features/lab-02/tests/AttachmentSection.test.tsx | Pass |
| UI-15 | UI | AC-06 | No requester selected opens guarded screen | Requester Selection screen shown | client/src/features/lab-02/tests/RequesterSelection.test.tsx | Pass |
| UI-16 | UI | AC-08 | Change Requester | Refetch of requester-specific data | client/src/features/lab-02/tests/RequesterSelection.test.tsx | Pass |
| UI-17 | UI | AC-07, AC-09 | Dropdown renders the backend's active-only requester response; empty & failure/Retry states render | API response renders as supplied; empty/failure states follow ui-spec §5.1. API-01, not the client, proves inactive exclusion | client/src/features/lab-02/tests/RequesterSelection.test.tsx | Pass |
| UI-18 | UI | AC-05, BR-12 | Busy label during submit | "Submitting…" + disabled | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-19 | UI | AC-28 | Component-level accessibility semantics/focus behavior: nav toggle aria state, first-invalid-field focus, removal modal label/Esc/focus return | Component semantics and focus behavior correct; full keyboard reachability/visible-focus evidence is `A11Y-01` | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| UI-20 | UI | AC-27 | Responsive proxy classes on mobile nav and Create actions | Mobile nav/stack classes applied; real-browser no-overflow evidence is `E2E-05` + `VISUAL-01` | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| UI-21 | UI | FR-02, FR-12 | Requester Selection loading spinner during fetch | Loading state renders before data arrives | client/src/features/lab-02/tests/RequesterSelection.test.tsx | Pass |
| UI-22 | UI | AC-25, FR-12 | Create Ticket reference-data loading and failure | Skeleton/spinner while loading; on failure Category/Related System stay disabled, Submit disabled, entered Summary/Description preserved (ui-spec §5.2) | client/src/features/lab-02/tests/CreateTicket.test.tsx | Pass |
| UI-23 | UI | FR-12, AC-08, AC-25 | My Tickets loading, requester-switch race protection, and category metadata failure/Retry | Latest requester wins; stale success/failure ignored; category failure does not hide loaded tickets | client/src/features/lab-02/tests/MyTickets.test.tsx | Pass |
| UI-24 | UI | AC-25 | Ticket Detail loading and API failure | Skeleton during load; safe banner + Retry on failure | client/src/features/lab-02/tests/RequesterTicketDetail.test.tsx | Pass |
| UI-25 | UI | FR-07, AC-14, AC-28 | My Tickets heading Clear Filters, accessible sortable headers/mobile sort controls, Ticket Number link, and row/card navigation | Reset/default behavior correct; sort state/query correct; no Action column; detail navigation remains accessible | client/src/features/lab-02/tests/MyTickets.test.tsx | Pass |
| UI-26 | UI | ui-spec §5.2, §5.4; AC-28 | Create Ticket + Ticket Detail top-right `Back to My Tickets` navigation | Real `/my-tickets` links render in the heading area; Create Back does not submit; Detail Back does not mutate Ticket/Attachments; controls remain keyboard-accessible | client/src/features/lab-02/tests/CreateTicket.test.tsx; client/src/features/lab-02/tests/RequesterTicketDetail.test.tsx | Pass |
| UI-27 | UI | ui-spec §5.3, AC-27 | My Tickets visible toolbar labels including Current Status, stable seven-column table plan, uniform muted-green header styling, neutral zebra rows, two-line Summary clamp, Created/Ticket Date display and sorting | Labels match accessible names; Current Status filter forwards `NEW`; Created sits after Ticket Number and sorts via `ticketDate`; fixed column plan does not depend on content length; full Summary remains in DOM while visual rows stay consistent with ellipsis; table header stays visually subordinate to the primary app header while remaining readable and keyboard focus remains visible | client/src/features/lab-02/tests/MyTickets.test.tsx; e2e/lab-02/accessibility.spec.ts | Pass |

### UI Style — `client/src/features/lab-02/tests/`

| ID | Type | Requirement / AC | What It Tests | Expected Result | Test File | Final |
|---|---|---|---|---|---|---|
| STYLE-01 | Style | ui-spec §1 | Header/primary actions use #006B3C token class | Token class present | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| STYLE-02 | Style | ui-spec §1, §4 | Selected Requester identity uses pale-green emphasis class #EAF6EF | Token class present on selected identity | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| STYLE-03 | Style | ui-spec §5.3 | Priority/status badge classes per value (LOW gray … CRITICAL red; NEW pale green) | Badge class mapping correct | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| STYLE-04 | Style | ui-spec §3 | Required-field asterisks on required labels | Asterisk element present | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
| STYLE-05 | Style | ui-spec §3 | Read-only vs editable field class distinction | Distinct classes applied | client/src/features/lab-02/tests/ui-style.test.tsx | Pass |
...
Final status rule: every row stayed `Planned` through feature/staging work and moved only after exact final-main verification. PR #37 was peer-merged into `main` as `967d8fed8f06a4240a34cb30498f3489a67c3ed0`. Hosted CI run `33877953656` ran on that exact merge SHA and completed **SUCCESS**. The server job passed migrations, seed, typecheck, guardrails, and **101/101** Vitest/Supertest tests. The client job passed typecheck and **67/67** Vitest tests. The E2E job completed successfully with **6 passed + 1 flaky**: `E2E-01` timed out once while waiting for the visible Official Ticket Number, then passed on Playwright's configured retry. Accordingly, `E2E-01` is recorded as `Pass (retry)` rather than presented as a clean first-pass result; the remaining planned rows are `Pass`.

The feature-branch paragraphs below preserve the evidence/status wording from the time each stage was recorded. Any historical statement that a `Final` cell remained `Planned` describes that earlier stage; the authoritative current final status is the table above plus this exact final-main record.
...
**Final release evidence (PR #37):** @thananun-7203 reviewed staging head `42ad3bc…`, found no production-code blocker, and submitted `CHANGES_REQUESTED` for documentation/evidence synchronization. A deeper `docs/lab-02` audit also corrected the stale `API-22` / `UI-12` / `UI-13` evidence paths with direct evidence-gap tests. @Tanaboonnnnn independently found that Issue #19 had been closed before the release gate and requested reopening it. The issue was reopened, documentation/process evidence was synchronized, and @Tanaboonnnnn re-reviewed exact final head `b9da7c49e869b378cb2f2536639aaccf12ae70f4` as **APPROVED**. The same peer merged PR #37 into `main` as `967d8fed8f06a4240a34cb30498f3489a67c3ed0`. Final-main CI run `33877953656` completed **SUCCESS** on that exact SHA with server **101/101**, client **67/67**, and E2E job success (**6 passed + 1 flaky**, with E2E-01 passing on retry). No production or test implementation changed between the approved release head and the merge tree.
...
- `STYLE-01`…`STYLE-05`, `UI-19`, and `UI-20` are no longer deferred: Issue 12 added their automated evidence, and their current `Final` status is `Pass` from exact final-main verification.
...
- Issue 13 added the previously missing `UNIT-01`…`UNIT-04`, `API-01`…`API-03`, `UI-15`…`UI-17`, and `UI-21` evidence. Their current `Final` status is `Pass` from exact final-main verification.
- The PR #37 release audit corrected stale `API-22` / `UI-12` / `UI-13` evidence paths by adding direct tests for the existing partial-upload and Create Ticket invalid-selection behavior; these rows now have final-main `Pass` evidence.
- Final-main `E2E-01` is a known reliability note rather than a hidden clean pass: it timed out once waiting for the visible Official Ticket Number and passed on Playwright retry in run `33877953656`. The E2E job still completed successfully; final submission evidence should state `6 passed + 1 flaky`, not `7/7` first-pass.
