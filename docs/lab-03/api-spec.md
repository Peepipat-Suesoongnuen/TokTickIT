...
- parent Ticket must belong to authenticated Requester;
...
- if status was `NEW`, `currentStatus = OPEN` in the same transaction;
- otherwise status is unchanged where claiming is valid under the current state contract.
...
POST request accepts only `{ "content": "..." }`, uses the same trim/plain-text/1–2,000 rules as Public Comments, and returns `201`.
...
- trim name; validate required supported length according to implementation constants documented/tests;
- trim + canonicalize email and validate email format;
