...
### Cross-feature concurrency and owner integrity

- **BR-76** Owner eligibility is a cross-feature invariant. Any operation that establishes or changes the owner of a non-terminal Ticket, including Claim, first Assign, Reassign, and Reopen owner repair, and any Administrator update that could make the same target User owner-ineligible (`isActive = false` or role changed away from `IT_STAFF`/`ADMINISTRATOR`) must use a shared concurrency-safe protocol that serializes the conflicting state or conditionally revalidates the affected User/Ticket state before commit. Both conflicting operations must never commit to an invalid final state; when they race, at least one operation is rejected with safe `409 Conflict`. After every committed operation, each non-terminal Ticket with a non-null owner references an active `IT_STAFF` or `ADMINISTRATOR`.
...
- **AC-09 Ownership:** Claim/assign/reassign accepts only eligible active IT Staff/Administrator owners. First assignment changes `NEW → OPEN` atomically. Concurrent Claim has one winner, stale ownership mutation is rejected, and concurrent Claim/Assign/Reassign versus target-user deactivation/demotion cannot commit an ineligible owner; at least one conflicting request fails safely with `409`.
...
- **AC-11 Status Workflow:** The backend accepts every listed permitted transition and rejects every unlisted/forbidden transition, including direct `NEW → OPEN` through the generic status endpoint. Reopen owner invariants, including commit-time eligibility of any replacement owner under BR-76, and Requester-resolution indication clearing are enforced atomically.
...
- **AC-14 Administrator Safety:** Self-deactivation, removal/deactivation/role-away of the last active Administrator, and deactivation/role-away of a User still owning non-terminal Tickets are rejected atomically. Concurrent Administrator deactivate/demote operations versus any owner-establishing mutation for that same target, including Claim/Assign/Reassign/Reopen owner repair, preserve the owner-eligibility invariant and reject at least one conflicting request with safe `409`.
...
13. **Concurrency:** use targeted expected-value/conditional mutations for Ticket operations rather than introducing a generic row-version framework throughout the application. Owner mutations and Administrator eligibility-changing updates additionally share a concurrency-safe owner-integrity protocol; the contract requires the invariant and safe conflict behavior, not one specific locking/transaction implementation.
