...
- **FR-15** Administrators shall list users, search by name/email, and optionally filter by role.
...
- **BR-08** Passwords are 8–64 characters, contain at least one uppercase letter, one lowercase letter, and one special character, and must differ from the current password. A digit is allowed but not required.
...
- **BR-38** Comment/note content length is 1–2,000 characters after trimming.
...
- Replace the Development Requester selector with Login and mandatory Change Password flows; replace the shell requester chip/change-requester action with authenticated user name, role, Change Password, and Logout.
...
- Requester Ticket Detail extends the Lab 2 screen with `Assigned To`, Public Comments, and Problem Appears Resolved; Internal Notes are not rendered at all.
- IT Staff Ticket Queue reuses the Lab 2 toolbar/table/mobile-card language while avoiding a mega-grid. The selected desktop columns are Ticket Number, Summary, IT Priority, Current Status, Ticket Owner, and Last Updated; Requester, Category, and Requested Priority appear as secondary metadata.
- IT Staff Ticket Detail extends the Lab 2 detail layout with an operational panel for Claim/Owner, IT Priority, and permitted Status plus visually distinct Public Comments and Internal Notes sections.
- Administrator User Management is intentionally simple: Name, Email, Role, Status, Edit; search name/email; optional role filter; Create/Edit dialogs; separate Set New Initial Password dialog.
...
`id`, `ticketId` FK, `authorId` FK → User, `content` (1–2,000 chars after trim), `createdAt` UTC. Append-only.
...
- **AC-12 Communication:** Public Comments and Internal Notes are append-only, backend-authored/timestamped, trimmed, 1–2,000-character plain text. Internal Notes are accessible only to IT Staff/Administrator and never leak through Requester responses.
- **AC-13 Administrator User Management:** Administrator can list/search/filter, create, and edit Users with exactly one role, case-insensitive unique email, activation state, and approved initial-password behavior; non-Administrators are forbidden.
...
7. **Staff Queue density:** desktop Queue uses Ticket Number, Summary, IT Priority, Current Status, Ticket Owner, and Last Updated as columns. Requester, Category, and Requested Priority remain visible as secondary row/card metadata to avoid an unreadable mega-grid.
...
10. **Requester resolution indication:** stored as nullable `requesterResolutionIndicatedAt`; setting it does not alter Current Status. It remains through Resolved/Closed and is cleared on Reopen.
