# Lab 3 Sprint Engineering Specification โ€” TokTickIT Authentication, Staff Workflow, and Administration
...
- Requester Public Comments and a โ€Problem Appears Resolvedโ€ indication that does not formally resolve or close a Ticket.
...
- Administrator Ticket operations explicitly permitted by this projectโ€s authorization matrix, plus Administrator-only minimalist User Management.
...
- Standalone administrator account-unlock workflow; setting a new initial password may clear the target accountโ€s login-lock state as part of that approved operation.
...
- **FR-12** Authorized staff-side users shall set and update IT Priority independently of the Requesterโ€s Requested Priority.
...
- **FR-15** Administrators shall list users, search by name/email, and filter by role and activation status.
...
- **FR-17** Administrators shall update a userโ€s name, email, role, and activation state and shall set a new initial password that requires change at the next login.
...
- **BR-08** Passwords are 8โ€“64 characters, contain at least one uppercase letter, one lowercase letter, and one special character, and must differ from the current password. A digit is allowed but not required.
...
- **BR-10** Passwords and initial passwords are never stored or logged in plaintext and are never returned in API responses or evidence. Password storage uses a password-specific slow salted hash; the planned implementation uses bcrypt with cost factor 12.
...
- **BR-21** `ADMINISTRATOR` remains a distinct single role. For this projectโ€s explicit authorization matrix, an Administrator also receives the IT Staff Ticket operational permissions in addition to Administrator-only User Management. This is an explicit project decision, not an assumed role hierarchy.
...
- **BR-27** The first successful Claim or owner assignment of a `NEW` Ticket atomically assigns the owner and changes `NEW โ’ OPEN`.
- **BR-28** `OPEN โ’ IN_PROGRESS` is an explicit IT Staff/Administrator action.
- **BR-29** Reassignment after the first assignment does not change Current Status.
- **BR-30** `IN_PROGRESS โ’ WAITING_FOR_REQUESTER` and `WAITING_FOR_REQUESTER โ’ IN_PROGRESS` are explicit IT Staff/Administrator actions. A Requester comment does not automatically change status.
- **BR-31** IT Staff/Administrator may resolve only from `IN_PROGRESS` or `WAITING_FOR_REQUESTER`.
- **BR-32** `RESOLVED โ’ CLOSED` is permitted. `RESOLVED โ’ REOPENED` and `CLOSED โ’ REOPENED` are permitted. `REOPENED โ’ IN_PROGRESS` is then explicit.
- **BR-33** IT Staff/Administrator may cancel from `NEW`, `OPEN`, `IN_PROGRESS`, `WAITING_FOR_REQUESTER`, or `REOPENED`. `CANCELLED` is terminal. Direct `RESOLVED โ’ CANCELLED` and `CLOSED โ’ CANCELLED` are forbidden; if necessary, a resolved Ticket must first be reopened. Cancellation means current work is intentionally discontinued and does not erase prior history.
...
- **BR-49** Migration preserves each Lab 2 `DevelopmentRequester โ’ Ticket` relationship when evolving Requesters into Users.
...
- **BR-55** TokTickIT is a first-party same-application web client. State-changing requests must come from the configured approved application origin. The session cookie uses `SameSite=Lax`, credentialed CORS is restricted to configured origins, and unrestricted credentialed CORS is forbidden. A separate synchronizer-CSRF-token subsystem is not required for Lab 3 under this architecture, but must be reconsidered if the architecture later requires cross-site authenticated requests.
...
- **BR-58** Login normalizes email, applies IP rate limiting, and for an existing account evaluates lock, credential, activation, failed-attempt counters, and session creation. Account-specific failures remain generic; a rate-limit response may be distinct.
...
- **BR-72** Credentialed CORS is environment-aware and allow-lists the configured application origin. Production/same-origin deployment should prefer same-origin frontend/API. `Access-Control-Allow-Origin: *` is not used with credentialed authentication.
...
| `CANCELLED` | โ€” | โ€” | Terminal |

`RESOLVED โ’ CANCELLED`, `CLOSED โ’ CANCELLED`, direct `NEW โ’ OPEN` through the generic status endpoint, `NEW/OPEN โ’ RESOLVED`, and all unlisted transitions are forbidden.
...
| Field | Type | Constraints |
|---|---|---|
| `requesterId` | Int | FK โ’ User; historical submitter; immutable |
| `ticketOwnerId` | Int? | FK โ’ User; zero/one operational owner |
...
`id`, `ticketId` FK, `authorId` FK โ’ User, `content` (1โ€“2,000 chars after trim), `createdAt` UTC. Append-only and queried only through Staff/Admin authorization paths.
...
- Migrate/evolve each `DevelopmentRequester` to a `User` while preserving the requester IDs or using an explicit deterministic mapping that keeps every existing Ticketโ€s requester relationship correct.
...
Seeded credentials are clearly documented as local-development/testing credentials only. Real personal passwords or secrets are never committed.
...
All protected operations use backend authentication/authorization. Resource existence is intentionally hidden with `404` where revealing another Requesterโ€s protected Ticket/Attachment would leak information.
...
- **AC-04 Requester Identity Security:** Requester Ticket/Attachment operations derive ownership from authenticated identity. Client-supplied requester identity cannot access another Requesterโ€s data, and unauthorized protected resources fail safely.
...
- **AC-06 Requester Lab 3 Features:** An authenticated Requester can see Assigned To/Unassigned in owned Ticket Detail, read/add Public Comments, and indicate Problem Appears Resolved without changing formal Current Status.
...
- **AC-09 Ownership:** Claim/assign/reassign accepts only eligible active IT Staff/Administrator owners. First assignment changes `NEW โ’ OPEN` atomically. Concurrent Claim has one winner and stale ownership mutation is rejected.
...
- **AC-11 Status Workflow:** The backend accepts every listed permitted transition and rejects every unlisted/forbidden transition, including direct `NEW โ’ OPEN` through the generic status endpoint. Reopen owner invariants and Requester-resolution indication clearing are enforced atomically.
...
- **AC-13 Administrator User Management:** Administrator can list/search/filter by role/status, refresh, create, and edit Users with exactly one role, case-insensitive unique email, activation state, and approved initial-password behavior; desktop rows/mobile cards open Edit User directly; non-Administrators are forbidden.
...
- **AC-16 Migration and Seed:** Lab 2 Requesters evolve into Users while existing Ticket/Attachment/reference-data relationships remain valid; IT Priority migration and local initial credentials are correct; required Lab 3 seed counts/data are repeatable without duplication.
...
1. **Authorization model:** RBAC is combined with object ownership and workflow-state rules. Subject + action + object/state determines authorization. Administratorโ€s Staff Ticket permissions are an explicit TokTickIT matrix decision rather than inheritance by convention.
...
3. **Password storage:** use a password-specific slow salted hash; implementation targets bcrypt cost factor 12. This is a TokTickIT implementation choice, not a requirement that the lecture fixed to one algorithm.
4. **CSRF/CORS:** first-party browser architecture uses `SameSite=Lax`, approved-origin checks for state-changing requests, and strict credentialed CORS. A dedicated synchronizer token is intentionally omitted for this architecture and must be reconsidered if cross-site authenticated use is introduced.
