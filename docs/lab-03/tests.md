...
| ID | Type | Requirement / AC | What It Tests | Expected Result | Final |
|---|---|---|---|---|---|
| API-38 | API | FR-15, AC-13 | Admin list/search/role/status filters | safe fields only; case-insensitive name/email search; exact role filter; `active=true|false` filter | Planned |
...
| UI-06 | UI | AC-06 | Requester Detail additions | detail retains My Tickets fields including Req. Priority/Status/Last Updated; Assigned To/Unassigned; breadcrumb; compact tabs; Public Comments; appears-resolved action; no Internal Notes | `RequesterTicketDetail.test.tsx` | Planned |
...
| UI-09 | UI | AC-12, AC-17 | Public vs Internal communication | explicit private label/lock semantic; composers stay above timelines; Public Comment 200 counter/validation; Internal Note 2000 counter/validation; compact aligned actions; no color-only distinction | `TicketCommunication.test.tsx` | Planned |
| UI-10 | UI | AC-13 | Admin list/create/edit/reset | Search/Role/Status filters + Clear/Refresh/Create; Name/Email/Role/Status columns; row/card opens Edit without Edit button; consistent role/status badges; required fields/one role; eye-icon password visibility; no password echo; responsive semantics | `AdminUsers.test.tsx` | Planned |
