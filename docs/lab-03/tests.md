...
|---|---|---|---|---|---|---|
| UNIT-01 | Unit | BR-08, AC-02 | password policy min/max, upper/lower/special, current-password difference | valid boundaries accepted; invalid values rejected deterministically | `server/src/lib/__tests__/password-policy.test.ts` | Planned |
...
| UNIT-05 | Unit | BR-36–BR-38, AC-12 | Public Comment/Internal Note trim + 1–2,000 validation | whitespace/0/>2000 rejected; valid text normalized | `server/src/lib/__tests__/message-validation.test.ts` | Planned |
...
| API-07 | API | BR-08, BR-12, BR-61, AC-02 | valid/invalid password change | valid change succeeds and clears mandatory state; invalid/wrong-current causes no partial change | Planned |
...
| API-17 | API | BR-34–BR-38, AC-06, AC-12 | own Public Comments GET/POST | append-only, backend author/time, trim + 1–2000, deterministic order | Planned |
| API-18 | API | BR-05, AC-06 | Problem Appears Resolved | indication timestamp set; `currentStatus` unchanged | Planned |
...
| API-35 | API | BR-34–BR-38, AC-12 | Staff/Admin Internal Notes GET/POST | append-only, backend author/time, trim/length, deterministic order | Planned |
...
| ID | Type | Requirement / AC | What It Tests | Expected Result | Final |
|---|---|---|---|---|---|
| API-38 | API | FR-15, AC-13 | Admin list/search/optional role filter | safe fields only; case-insensitive name/email search; exact role filter | Planned |
| API-39 | API | FR-16, BR-39–BR-40, AC-13 | create user | exactly one role; canonical unique email; hashed initial password; `mustChangePassword=true` | Planned |
...
| API-45 | API | BR-44, BR-62, AC-15 | set new initial password | hash changed, mandatory change true, lock reset, all target sessions revoked, no secret response | Planned |
...
| SEC-06 | Security | BR-55, BR-72, AC-18 | state-changing request from unapproved Origin / credentialed CORS config | rejected/not credential-authorized per environment contract; approved origin works | `server/tests/lab-03/security.api.test.ts` | Planned |
...
| MIG-04 | Migration | BR-52, AC-16 | migrated Requester credential state | password is non-plaintext hash; `mustChangePassword=true`; local credential rule documented | Planned |
| MIG-05 | Migration / seed | AC-16 | idempotent seed + minimum Lab 3 accounts | ≥4 active +1 inactive Requester; ≥3 active +1 inactive Staff; ≥1 active Admin; rerun no duplicates | Planned |
...
| UI-02 | UI | AC-02 | Mandatory Change Password | password requirements/confirmation; invalid validation; busy; success continuation; Logout | `ChangePassword.test.tsx` | Planned |
...
| UI-04 | UI / regression | AC-05 | Create Ticket without selector | authenticated Requester shown read-only; Lab 2 validation/busy/success/failure preserved | `RequesterCreateTicket.test.tsx` | Planned |
| UI-05 | UI / regression | AC-05 | My Tickets with eight statuses | Lab 2 columns/search/filter/sort/pagination/card states preserved; no Owner column | `RequesterMyTickets.test.tsx` | Planned |
| UI-06 | UI | AC-06 | Requester Detail additions | Assigned To/Unassigned, Public Comments, appears-resolved action; no Internal Notes | `RequesterTicketDetail.test.tsx` | Planned |
| UI-07 | UI | AC-08, AC-17 | Staff Queue | filters/sort/table/mobile-card metadata/loading/empty/no-results/forbidden/failure | `StaffQueue.test.tsx` | Planned |
| UI-08 | UI | AC-09–AC-11 | Staff Detail operations | Claim/Owner/IT Priority/status controls reflect state; stale conflict refresh feedback | `StaffTicketDetail.test.tsx` | Planned |
| UI-09 | UI | AC-12, AC-17 | Public vs Internal communication | explicit private label/lock semantic; separate inputs; 2000 counter/validation; no color-only distinction | `TicketCommunication.test.tsx` | Planned |
| UI-10 | UI | AC-13 | Admin list/create/edit/reset dialogs | required fields, one role, validation, no password echo, responsive list/card semantics | `AdminUsers.test.tsx` | Planned |
...
| AC-02 Mandatory Password Change | UNIT-01, API-06–07, UI-02, E2E-01 |
...
| AC-15 Initial Password Reset | UNIT-01, UNIT-06, API-10, API-45, E2E-05 |
| AC-16 Migration and Seed | MIG-01–06 |
| AC-17 UI/UX | UI-01–13, STYLE-01–04, A11Y-01, VISUAL-01 |
| AC-18 Safe Failure and Security | API-23, SEC-01–10, UI-12, E2E-06, VISUAL-01 |
...
|---|---|
| #44 User migration/schema/seed | `migration-regression.test.ts`, migration/seed helpers |
| #45 Authentication | `auth.api.test.ts`, password/session/login-protection unit tests, Login/ChangePassword UI tests |
...
- [ ] Staff Queue uses the approved six-column information hierarchy and readable mobile cards; no mega-grid/horizontal page scroll.
...
- [ ] Admin list/create/edit/reset works desktop/mobile; dialogs fit viewport.
