...
| API-15b | API | BR-51, AC-07 | Staff/Admin attachment read/download on authorized Ticket | Staff/Admin `GET /api/attachments/:id` metadata + `/download` succeed on visible Ticket; upload/remove remain denied | Planned |
...
| MIG-02b | Migration | BR-40, BR-46, AC-16 | normalized-email collision aborts safely | fixture `Alice@x` + `alice@x` → migration fails before any mutation; no partial FK rewrite, no merged accounts | Planned |
...
| AC-07 Authorization | API-15b, API-19, API-24, API-34, API-46, SEC-01, UI-03, E2E-06 |
